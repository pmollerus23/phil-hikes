...
import { type Trip, type TripDetail, type Waypoint, type Point } from './model';
  ...
  <aside className="trip-panel" aria-label="Trip explorer">
   ...
   <div id="trip-panel-content" ref={panel} className="panel-content" hidden={!open}>
    ...
    {selected?<>
     ...
     {waypoint&&<section className="waypoint-detail" aria-label="Waypoint details" aria-live="polite"><button className="close-detail" aria-label="Close waypoint details" onClick={()=>setWaypoint(null)}>×</button><span className="eyebrow"><WaypointIcon kind={waypoint.kind} /> {waypoint.kind.replace('-',' / ')}{waypoint.derived?' · derived':''}</span><h2>{waypoint.name}</h2><p>{waypoint.description||'No description supplied.'}</p>{(waypoint.symbol||waypoint.type)&&<p className="caption">Original symbol/type: {waypoint.symbol||waypoint.type}</p>}</section>}
     ...
     <section className="waypoint-list"><div className="section-label">ALONG THE WAY <span>{selected.waypoints.length} places</span></div>{selected.waypoints.map(w=><button key={w.id} aria-pressed={waypoint?.id===w.id} onClick={()=>{setWaypoint(w);showPoint(w);panel.current?.scrollTo({top:0});}}><span className={`list-icon kind-${w.kind}`} aria-hidden="true"><WaypointIcon kind={w.kind} /></span><span>{w.name}<small>{w.kind.replace('-',' / ')}{w.derived?' · derived':''}</small></span><span aria-hidden="true">↗</span></button>)}</section>
