...
export function parseGpx(xml: string, filename: string, override: Override = {}) {
 ...
 for(const [tag,kind] of [['trk','track'],['rte','route']] as const) {
  array(root[tag]).forEach((raw,i)=>{
   ...
   paths.push({id:pathId,kind,name:str(raw.name)||`${kind} ${i+1}`,description:str(raw.desc),extensions:raw.extensions??null,segments});
 ...
 if(paths.filter(p=>p.kind==='route').length>1) issues.push('Multiple planned routes may overlap or be alternatives; totals are mapped geometry, not measured travel.');
 ...
 const ps=paths.flatMap(p=>p.segments.flatMap(s=>s.points));
 const inventory={id,filename,tracks:paths.filter(p=>p.kind==='track').length,routes:paths.filter(p=>p.kind==='route').length,trackSegments:paths.filter(p=>p.kind==='track').reduce((n,p)=>n+p.segments.length,0),routeSegments:paths.filter(p=>p.kind==='route').reduce((n,p)=>n+p.segments.length,0),emptySegments:paths.flatMap(p=>p.segments).filter(s=>!s.points.length).length,sourceWaypoints:array(root.wpt).length,campsites:waypoints.filter(w=>w.kind==='camp'&&!w.derived).length,derivedMarkers:waypoints.filter(w=>w.derived).length,points:ps.length,elevationPoints:ps.filter(p=>p.elevation!==null).length,timestampPoints:ps.filter(p=>p.time!==null).length,issues};
