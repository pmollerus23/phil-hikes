...
- Summary paths default to recorded tracks if present, otherwise planned routes. Overrides can choose an ordered subset for statistics/profile without deleting alternative geometry. Maine publishes only the continuous “Maine AT Section Route” (`rte-10`) for its map and profile; older itineraries and day plans remain in the original export and source inventory. Beartown and LSPP still have potentially overlapping/approximate plans and are explicitly labeled.
