...
// Keep the directory in place so the dev server can track regenerated assets.
await mkdir('public/trips',{recursive:true});
for(const r of results) await rename(`${staging}/${r.trip.id}.json`,`public/trips/${r.trip.id}.json`);
await rename(`${staging}/inventory.json`,'public/trips/inventory.json');
await rename(`${staging}/index.json`,'public/trips/index.json');
const publishedFiles=new Set([...ids.map(id=>`${id}.json`),'index.json','inventory.json']);
for(const filename of await readdir('public/trips')) if(!publishedFiles.has(filename)) await rm(`public/trips/${filename}`,{recursive:true,force:true});
