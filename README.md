...
Optional `pathIds` chooses an ordered subset of source paths to publish in the map and detail asset. Bounds, default statistics, profiles, and derived endpoints use these published paths; source GPX files and inventory counts retain all paths. Any explicit `statsPathIds` must reference published paths.
...
- Summary paths default to recorded tracks if present, otherwise planned routes. Overrides can choose an ordered subset for statistics/profile without deleting alternative geometry. Maine publishes only the continuous “Maine AT Section Route” (`rte-10`) for its map and profile; older itineraries and day plans remain in the original export and source inventory. Johnson Lake publishes its updated loop (`rte-1`). LSPP publishes only the updated portage route (`rte-3`); its truck waypoint marks the trip's start/end. The fishing route and old portage route are excluded from its map, statistics, and profile. Beartown still has potentially overlapping plans and is explicitly labeled.
...
Source baseline: **12 trips, 0 tracks, 25 routes (one empty), 31,717 geometry points, 42 source waypoints, 26 campsites**. Every geometry point has elevation and a timestamp, but each nonempty route repeats one timestamp. Counts reconcile exactly with independently inspected XML. LSPP coordinates appear to be in Ontario, contrary to the original US-only description; they are preserved and flagged for owner review. Beartown’s plan extends beyond its marked start; Maine's export includes overlapping day plans and an empty second route, which are excluded from its published trip. Johnson Lake and LSPP retain older route versions in their source exports; presentation overrides choose their updated routes. Shenandoah has no reliable trip date.
