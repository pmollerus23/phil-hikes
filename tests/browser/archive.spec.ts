...
test('direct link loads trip, photo carousel, profile, and selectable waypoint descriptions',async({page})=>{
 await page.goto(`/map?trip=${id}`);
 ...
 await expect(page.locator('.trip-footer')).toContainText('Rangeley to Flagstaff');
 await page.getByRole('button',{name:'ⓘ Trip info'}).click();
 await expect(page.getByRole('heading',{name:'Rangeley to Flagstaff',exact:true})).toBeVisible();await expect(page.getByRole('slider',{name:'Explore elevation profile'})).toBeVisible();
...
test('late detail response cannot replace a newer selection',async({page})=>{
 ...
 await page.goto('/map');await page.getByRole('button',{name:/01 Beartown/}).click();await page.getByRole('button',{name:'← All trips'}).click();await page.getByRole('button',{name:/Rangeley to Flagstaff Maine/}).click();await expect(page.getByRole('list',{name:'Trip photos in route order'}).locator('.carousel-item')).toHaveCount(86);await page.getByRole('button',{name:'ⓘ Trip info'}).click();await expect(page.getByRole('slider')).toHaveAttribute('max','3440');await expect(page.getByRole('heading',{name:'Rangeley to Flagstaff',exact:true})).toBeVisible();
});
...
test('photo demo connects trip carousel, place detail, captions, viewer keyboard, history, and failure fallback',async({page,isMobile})=>{
 await page.goto('/photo-demo?trip=little-rock-creek-lake-mt-2024');
 const carousel=page.getByRole('list',{name:'Trip photos in route order'});
 await expect(carousel.locator('.carousel-item')).toHaveCount(3);
 await expect(carousel.locator('.carousel-item').first().locator('img')).toHaveAttribute('src','/demo/photos/lake-evening-thumb.svg');
 const footer=page.locator('.trip-footer');await expect(footer).toContainText('Little Rock Creek Lake');await expect(footer).toContainText('3 photos');
 await expect.poll(()=>page.locator('.carousel-photo img').evaluateAll(images=>images.every(image=>(image as HTMLImageElement).complete&&(image as HTMLImageElement).naturalWidth>0))).toBe(true);
 await page.screenshot({path:`test-results/photo-gallery-${test.info().project.name}.png`,fullPage:true});
 const galleryOpener=carousel.getByRole('button',{name:/Enlarge photo 1/});await galleryOpener.click();
 ...
 await page.route('**/demo/photos/creek-crossing.svg',route=>route.abort());await page.goto('/photo-demo?trip=little-rock-creek-lake-mt-2024');await carousel.getByRole('button',{name:/Enlarge photo 3/}).click();await expect(viewer.getByText('Image unavailable')).toBeVisible();
});
...
test('Maine trip loads its authored GPS photo carousel and full-size images on demand',async({page})=>{
 await page.goto('/map?trip=maine-august-2026');
 const carousel=page.getByRole('list',{name:'Trip photos in route order'});
 await expect(carousel.locator('.carousel-item')).toHaveCount(86);
 const first=carousel.getByRole('button',{name:/Enlarge photo 1:/});await expect(first.locator('img')).toHaveAttribute('src','/photos/maine-august-2026/maine-at-photo-001-thumb.webp');
 await expect(page.locator('.trip-footer')).toContainText('86 photos');
...
test('Vermud trip loads its authored GPS photo carousel in route order',async({page})=>{
 await page.goto('/map?trip=vermud-2021');
 const carousel=page.getByRole('list',{name:'Trip photos in route order'});
 await expect(carousel.locator('.carousel-item')).toHaveCount(32);
 const first=carousel.getByRole('button',{name:/Enlarge photo 1:/});await expect(first.locator('img')).toHaveAttribute('src','/photos/vermud-2021/vermud-photo-01-thumb.webp');
